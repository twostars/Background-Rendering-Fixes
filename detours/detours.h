...
#define DETOUR_TRAMPOLINE_PTR(trampoline,target,ptr) \
static PVOID __fastcall _Detours_GetVA_##target(VOID) \
{ \
    return (PVOID)(ptr); \
} \
...
#define DETOUR_TRAMPOLINE_TO_ADDRESS(trampoline,hookName,target,ptr) \
DETOUR_TRAMPOLINE_PTR(trampoline,hookName,ptr); \
__declspec(naked) void hookName() { \
	__asm { JMP [target] }; \
}

#define	DETOUR_TRAMPOLINE_CLASS_METHOD DETOUR_TRAMPOLINE_TO_ADDRESS
